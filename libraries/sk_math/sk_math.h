...
template <typename T>
bool ISCLOSE(const T value, const T target, const T error) {
    return !(value < (target - error)) && (value < (target + error));
}

// returns true if value is positive
template <typename T>
bool ISPOS(const T value) {
...
template <typename T>
T SMOOTLERP(const T min, const T max, const T interpolationPoint) {
    return T(min + ((interpolationPoint * interpolationPoint) *
