...
// Source https://github.com/tomstewart89/BasicLinearAlgebra
...
template <int rows, int cols, class MemT>
template <class opMemT>
Matrix<rows, cols, MemT> &Matrix<rows, cols, MemT>::operator=(
    const Matrix<rows, cols, opMemT> &obj) {
...
template <int rows, int cols, class MemT>
Matrix<rows, cols, MemT> &Matrix<rows, cols, MemT>::operator=(
    typename MemT::elem_t arr[rows][cols]) {
    for (int i = 0; i < rows; i++)
...
template <int rows, int cols, class MemT>
Matrix<rows, cols, MemT> &Matrix<rows, cols, MemT>::Fill(
    const typename MemT::elem_t &val) {
    for (int i = 0; i < rows; i++)
...
template <int rows, int cols, class MemT>
typename MemT::elem_t Matrix<rows, cols, MemT>::operator()(int row,
                                                           int col) const {
    return storage(row, col);
}
...
Matrix<subRows, subCols, Reference<MemT>> Matrix<rows, cols, MemT>::Submatrix(
    int top, int left) {
...
template <int rows, int cols, class MemT>
template <int subRows, int subCols>
Matrix<subRows, subCols, ConstReference<MemT>>
Matrix<rows, cols, MemT>::Submatrix(int top, int left) const {
...
template <int rows, int cols, class MemT>
Matrix<1, cols, ConstReference<MemT>> Matrix<rows, cols, MemT>::Row(
    int i) const {
    return Submatrix<1, cols>(i, 0);
}
...
template <int rows, int cols, class MemT>
Matrix<rows, 1, ConstReference<MemT>> Matrix<rows, cols, MemT>::Column(
    int j) const {
    return Submatrix<rows, 1>(0, j);
}
...
Matrix<rows, cols, MemT>::operator||(
...
Matrix<rows, cols, MemT>::operator*(
    ...
    for (int i = 0; i < rows; i++)
        for (int j = 0; j < operandCols; j++) {
            if (cols > 0)
                ret.storage(i, j) = storage(i, 0) * operand.storage(0, j);

            for (int k = 1; k < cols; k++)
                ret.storage(i, j) += storage(i, k) * operand.storage(k, j);
        }
...
template <class opMemT>
Matrix<rows, cols, MemT> &Matrix<rows, cols, MemT>::operator*=(
    const Matrix<rows, cols, opMemT> &operand) {
...
template <int rows, int cols, class MemT>
Matrix<rows, cols, Array<rows, cols, typename MemT::elem_t>>
Matrix<rows, cols, MemT>::operator-() const {
...
template <int rows, int cols, class MemT>
Matrix<rows, cols, Array<rows, cols, typename MemT::elem_t>>
Matrix<rows, cols, MemT>::operator+(const typename MemT::elem_t k) const {
    Matrix<rows, cols, Array<rows, cols, typename MemT::elem_t>> ret(*this);
    ret += k;
    return ret;
}
